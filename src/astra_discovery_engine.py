...
class AstraDiscoveryEngine:
    ...
    def cross_match_with_gaia(self, transients, radius=5.0):
        ...
        for idx, row in has_coords.iterrows():
            try:
                    ...
                    results.append(
                        {
                            ...
                            "g_mag": (
                                ...
                                if "phot_g_mean_mag" in star.keys()
                                else None
                            ),
                        }
                    )
                ...
                results.append({"id": row["id"], "gaia_match": False, "error": str(e)})
    ...
    def find_anomalies(self, transients):
        ...
        for i, anomaly in enumerate(anomalies[:5], 1):
            print(
                f"   {i}. {anomaly['id']} (score={anomaly['score']:.1f}) - {', '.join(anomaly['reasons'])}"
            )
    ...
    def generate_discovery_report(self, anomalies):
        ...
        for i, obj in enumerate(anomalies, 1):
            ...
            report.append(
                f"   Magnitude: {obj['mag']:.1f}"
                if pd.notna(obj["mag"])
                else "   Magnitude: Unknown"
            )
    ...
    def run_discovery_pipeline(self, days=7):
        ...
        return {"transients": transients, "anomalies": anomalies, "report": report}
