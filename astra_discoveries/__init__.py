...
def _render_summary(results: dict, mode: str) -> str:
    ...
    lines.append("  • Review the report for observation targets")
    lines.append("  • Schedule follow-up or re-run with --advanced/--basic as needed")
...
def _run_quick_test(verbose: bool = False) -> int:
    ...
    print("🔍 Running ASTRA self-test (no network calls)...")
    ok = system_check()
    if not ok:
        print("❌ System check failed. See messages above.")
        return 1

    try:
        if not callable(run_basic_discovery):  # pragma: no cover - defensive
            raise RuntimeError("run_basic_discovery is not callable")
        print("✅ Core discovery functions available")
    except Exception as exc:  # pragma: no cover - defensive
        print(f"❌ Unable to import discovery pipeline: {exc}")
        return 1

    print("🎉 ASTRA installation looks good. Run with --advanced to begin discovering!")
    return 0
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if artifacts.catalog:
        print(f"  • Catalog: {artifacts.catalog}")
